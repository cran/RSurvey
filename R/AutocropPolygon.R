AutocropPolygon <- function(parent=NULL) {
  ...
  DrawPlot <- function() {
    ...
    if (is.na(max.len) || is.na(max.itr)) 
      return()
  ...
  Reset <- function() {
    tclvalue(max.len.var) <- default.len
    if (!is.null(dev)) {
      dev.off(which=dev)
      dev <<- NULL
      ply <<- NULL
  ...
  x2 <- c(x1[-1], x1[1])
  y2 <- c(y1[-1], y1[1])
  default.len <- format(max(sqrt((x2 - x1)^2 + (y2 - y1)^2)))
  
  # Assign the variables linked to Tk widgets
  
  max.len.var <- tclVar(default.len)
  max.itr.var <- tclVar(100000)
  ...
  frame0.but.2.1 <- ttkbutton(frame0, width=10, text="Build", 
                              command=DrawPlot)
  frame0.but.2.2 <- ttkbutton(frame0, width=10, text="Save", 
                              command=function() {
  ...
  frame0.but.2.3 <- ttkbutton(frame0, width=10, text="Cancel", 
                              command=function() tclvalue(tt.done.var) <- 1)
  
  frame0.but.2.4 <- ttkbutton(frame0, width=10, text="Reset", 
                              command=Reset)
