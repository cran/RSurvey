"polyTranslate" <- function(file=NULL, dl=NULL) {
...
# main program
  ...
  # import data from polygon file
    ...
    f <- getFile(cmd="Open", exts=c("ply"), file=file, caption="Open Polygon File")
    if(is.null(f)) return()
    
    con <- if("connection" %in% class(file)) file 
           else file(f$path, "r", encoding=srvy.dat("encoding"))
    
    dat <- readFile(con, fields=TRUE, units=FALSE, sep=srvy.dat("sep"), encoding=srvy.dat("encoding"))$dat
